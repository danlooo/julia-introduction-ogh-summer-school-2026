...
::: {.column width="60%"}
- Compiling: faster execution (cache binary creation) but slower start up
- Compiling is very uncommon in interactive terminals
- GCC compiles on every file change
- Julia JIT compiles on statement change, e.g. just needs to re-compile the `if` block
...
## Julia Compiler: Yet Another LLVM Language

Like C, C++, GO, Rust, ..., the default compiler for Julia uses LLVM infrastructure. 

![](assets/llvm.png)

On that low level, all those languages were the same.
...
Type e.g. `?println` and press enter to see help page of the function:
...
This is similar to R: `c(1,2,3) * 2`
...
Macros can modify code before compilation:
...
- Often used to automatically write repetitive code
...
The core interpreter in R is single-threaded, but Julia offers multi-threading by by default.
...
This allows us to run multiple tasks in parallel while working on *shared memory*.
In multi-processing, however, one needs to copy the runtime in memory multiple times.

## Multithreading

Executing a `for` loop in parallel (unsafe):
...
Concurrent reads are fine but we need to ensure that there is max 1 write at once.
...
- Multiprocessing is running multiple Julia instances without sharing memory.
- Those are running independently, even on different machines
